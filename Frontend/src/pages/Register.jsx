...
export default function Register() {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      const response = await axios.post(sendOtpRoute, { email: values.email });
      console.log("OTP Sent Response:", response.data);
      setShowOtpVerification(true);
      toast.success("OTP sent to your email", toastOptions);
    } catch (error) {
      console.error("Error sending OTP:", error.response?.data || error.message);
      toast.error("Server Error: Unable to send OTP", toastOptions);
    }
  };
          ...
          <form action="" onSubmit={(event) => handleSubmit(event)}>
            ...
            {showOtpVerification && (
              <div className="flex flex-col mb-3">
                ...
                <button
                  type="button"
                  className="btn bg-green-500 text-white p-2 mt-2 rounded-md"
